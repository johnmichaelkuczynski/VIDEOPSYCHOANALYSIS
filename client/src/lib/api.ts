...
export interface ComprehensiveAnalysis {
  ...
  psychologicalAnalysis?: Record<string, any>;
  overallSummary?: {
...
export interface PsychologicalParameter {
  id: string;
  name: string;
  description: string;
}
...
// New 6-Protocol Evaluation System
...
export async function analyzeTextWithProtocols(
  text: string,
  ...
  console.log(`Analyzing text with protocols: ${protocols.join(', ')}, model: ${selectedModel}, sessionId: ${sessionId}`);
  
  const response = await fetch("/api/analyze/protocols", {
    method: "POST",
  ...
  if (!response.ok) {
    ...
    throw new Error(errorText || "Protocol analysis failed");
  }
  
  const data = await response.json();
  console.log("Protocol analysis response:", data);
